...
class xpDebug {
	...
	function b($x, $name = '', $display = true, $tab = 6, $deep = 50, $level = 0) {
		...
		if (is_array($x) || is_object($x)) {
			foreach ($x as $k => $v) {
				...
				$con.= xpDebug::b($v, '', $display, $tab, $deep, $level);
		...
		if ($level == 1) {
			...
			if ($display) {
				...
				foreach (debug_backtrace() as $k => $v) {
					//	if($v['class'] == 'xpDebug' || $v['function'] =='_debug') continue;
	...
	function tree($x, $name = '', $close = 50, $deep = 50, $level = 0) {
		...
		if (is_array($x) || is_object($x)) {
			...
			foreach ($x as $k => $v) $con.= self::tree($v, $k, $close, $deep, $level + 1);
			$con.= "</ul></li>";
		} else {
