 ...
 * Last Modified..: $Date: 2006/08/17 06:21:51 $
 * CVS Revision...: $Revision: 1.1 $
 * Copyright......: 2001, 2002 Richard Heyes
 */
require_once (dirname(__FILE__) . '/htmlMimeMailPart.php');
class htmlMimeMail {
	/**
	...
	function _findHtmlImages($images_dir) {
		...
		preg_match_all('/(?:"|\')([^"\']+\.(' . preg_quote(implode('|', $extensions)) . '))(?:"|\')/Ui', $this->html, $images);
	...
	function send($recipients, $type = 'mail') {
		...
		switch ($type) {
			case 'mail':
				...
				// Return
				return array($result, $to, $subject, $this->output, implode(CRLF, $headers));
			break;
			case 'smtp':
