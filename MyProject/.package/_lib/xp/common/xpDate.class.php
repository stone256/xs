...
class xpDate {
	 ...
	 * $date,$start = today if not supplied,
	 * $end = next date of start if not supplied,
	 * $type = 0:> <; 1:>= <; 2:> <=; 3:>= <=;
	 */
	function in_range($start = null, $end = null, $date = null, $type = 0) {
	...
	function mytime() {
		return preg_replace('/0\.(\d+?)\s+(\d+)/', '$2.$1', microtime());
	}
	/**
	...
	function lastNsecond($date, $n) {
		return self::returnMYSQLTimestampFromUNIX(strtotime($date . " -" . (int)$n . " second"));
	}
	...
	function lastNminute($date, $n) {
		return self::returnMYSQLTimestampFromUNIX(strtotime($date . " -" . (int)$n . "  minute"));
	}
	...
	function lastNhour($date, $n) {
		return self::returnMYSQLTimestampFromUNIX(strtotime($date . " -" . (int)$n . "  hour"));
	}
	...
	function lastNdate($date, $n) {
		return self::returnMYSQLTimestampFromUNIX(strtotime($date . " -" . (int)$n . " day"));
	}
	...
	function lastNmonth($date, $n) {
		return self::returnMYSQLTimestampFromUNIX(strtotime($date . " -" . (int)$n . " month"));
	}
	...
	function get_time_string($str) {
		return xpAS::preg_get($str, '/[([0|1][0-9])|(2[0-3])] : [0-9][0-9] /');
	}
	/**
	...
	function num_days($d1, $d2, $type = 1) {
		switch ($type) {
			case 0:
				return self::_numberDays0($d1, $d2);
			break;
			case 1:
				return self::_numberDays1($d1, $d2);
			break;
			case 2:
	...
	function date_format($time, $t1, $t2) {
		$fmt = array(array(2, 1, 0), array(0, 1, 2), array(1, 0, 2));
		$time = explode(" ", $time);
		$d = preg_split('/[^\d]+/', $time[0]);
		$f = $fmt[$t1];
	...
	function period($d1, $d2, $type = null) {
		...
		switch ($type) {
			case 1:
				return sprintf("%02d", $a['hour']). ':' .sprintf("%02d", $a['minute']). ':' .sprintf("%02d", $a['second']);
			break;
			default:
				return $a;
			break;
	...
	function to12hours($time, $s = 0) {
		if (!($time = preg_replace('/\s*/', '', xpAS::preg_get($time, '/\d\d(\:[0-5][0-9]){1,2}/i')))) return $s ? '00:00:00AM' : '00:00AM';
		$hr = xpAS::preg_get($time, '/\d\d/');
		$hr = $hr % 24;
		$m = ($hr >= 12) ? 'PM' : 'AM';
		if ($hr > 12) $hr = $hr - 12;
