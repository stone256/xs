 ...
 *
 */
class api_userController extends sitemin_indexController {
	...
	function indexAction(){
		$rs = _factory('api_model_user')->gets($this->q);
		$rs['tpl'] = '/api/view/_api_user.phtml';
	...
	function passwordAction(){
		...
		if($q['_token'] == $_token && $q['id']){
			_factory('api_model_user')->setpassword($q);
		}
		die('done');
	}
	...
	function status_changeAction() {
		...
		$_token = defaultHelper::page_hash_get('api,user,list');
		if($q['_token'] == $_token && $q['id']){
			$r =  $q['status'] == 'active' ? _factory('api_model_user')->active($q) : _factory('api_model_user')->suspend($q);
			die( "ok");
		}
		echo "error..";
	}
	...
	function editAction() {
		...
		$rs = array();
		if($q['_token'] == $_token &&  $q['save'] && !($err = _factory('api_model_user')->save($q))) return defaultHelper::return_url(true);

		$q['_msg'] = $err ? implode(',', $err) : null;

		if((int)$q['id']){
			$rs = _factory('api_model_user')->get(array('id'=>$q['id']));
		}
		...
		$rs['status_options'] = _factory('api_model_user')->status;
		$rs['tpl'] = '/api/view/_api_user_edit.phtml';
	...
	function idcheckAction(){
		...
		die( _factory('api_model_user')->idcheck($this->q) ? 'error' : 'ok' );
	}
	...
	function searchAction($q){
		...
		$r = _factory('api_model_user')->search($q['term'], $q['type']);
